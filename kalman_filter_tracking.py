...
class KalmanFilter:
    ...
    def update(self, position_measurement, velocity_measurement=None):
        ...
        # Prediction
        A = np.array([[1, dt], [0, 1]])  # Motion model
        self.state = A @ self.state
        self.P = A @ self.P @ A.T + self.Q

        # Use position and optional velocity for Kalman update
...
class ObjectTracker:
    def __init__(self, speed_limit_kmh=1, speed_limits_map=None):
        ...
        self.match_tolerance = 0.35 # meters
        self.visual_tracks = {}  # visual ID -> track
    ...
    def _generate_id(self, radar_distance, velocity=0.0):
        rounded_dist = round(radar_distance, 1)
        rounded_vel = round(velocity, 1)
        ...
        if yolo_detections and frame_timestamp:
            ...
            for det in yolo_detections:
                ...
                if best_match:
                    ...
                    trk['score'] = self.calculate_score(trk)  # add score
        ...
        for det in detections:
            ...
            if not matched_id:
                ...
                self.trackers[matched_id] = KalmanFilter(
                    initial_position=det['radar_distance'],
                    initial_velocity=det.get('velocity', 0.0)
                )
            ...
            if velocity_measured is not None and abs(velocity_measured) > 0.01:
                tracker.update(det['radar_distance'], velocity_measured)
            else:
