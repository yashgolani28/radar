...
def fetch_training_data():
    query = """
        SELECT speed_kmh, radar_distance, velocity, signal_level, doppler_frequency, type 
        FROM radar_data
...
def train_and_save_model(rows):
    ...
    for row in rows:
        speed, dist, vel, signal, doppler, label = row
        X.append([speed, dist, abs(vel), signal, doppler])
        y.append(label.strip().upper())

    X = pd.DataFrame(X, columns=["speed_kmh", "radar_distance", "velocity", "signal_level", "doppler_frequency"])
    y = np.array(y)
